  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? 'bg-cream-50/95 backdrop-blur-md shadow-lg shadow-gold-900/5 py-3'
          : 'bg-transparent py-5'
      }`}
    >
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-5 sm:px-8 lg:px-12">
        ...
        <a
          href="#contact"
          className={`hidden rounded-full px-6 py-2.5 text-sm font-semibold transition-all duration-300 lg:inline-block ${
            scrolled
        ...
        <ul className="mx-5 mt-3 space-y-1 rounded-2xl bg-cream-50 p-4 shadow-xl">
          {navLinks.map((link) => (
            <li key={link.href}>
              <a
                ...
                className="block rounded-lg px-4 py-3 text-sm font-medium text-charcoal-700 transition-colors hover:bg-gold-100 hover:text-gold-700"
              >
                {link.label}
              </a>
            </li>
          ...
          <li>
            <a
              href="#contact"
              onClick={() => setMenuOpen(false)}
              className="mt-2 block rounded-lg bg-gold-600 px-4 py-3 text-center text-sm font-semibold text-white"
            >
              Book Now
            </a>
          </li>
